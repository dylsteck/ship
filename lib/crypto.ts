...
export const encrypt = (text: string): string => {
  ...
  if (!ENCRYPTION_KEY) {
    throw new Error(
      'ENCRYPTION_KEY environment variable is required for encryption. Generate one with: openssl rand -hex 32',
    )
  }
...
export const decrypt = (encryptedText: string): string => {
  ...
  if (!ENCRYPTION_KEY) {
    throw new Error(
      'ENCRYPTION_KEY environment variable is required for decryption. Generate one with: openssl rand -hex 32',
    )
  }
