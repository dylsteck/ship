/**
 * Generate a short session title from the first user prompt (and optional assistant preview).
 *
 * Tries Anthropic first, then OpenAI — REST only (no AI SDK dependency).
 */
...
async function generateViaAnthropic(userContent: string, apiKey: string): Promise<string | null> {
...
async function generateViaOpenAI(userContent: string, apiKey: string): Promise<string | null> {
...
export async function generateSessionTitle(options: GenerateTitleOptions): Promise<string | null> {
  const { userPrompt, assistantPreview, anthropicApiKey, openaiApiKey } = options
